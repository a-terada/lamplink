...
{  
  ...
  vector<int> snplist;
  map<int,vector_t>::iterator ti = rwin.begin();
  while ( ti != rwin.end() )
    {
      snplist.push_back( ti->first );
      ++ti;
    }
...
void displayScoresRegion(ofstream & O, RCount & rc)
{
  ...
  map<int,vector<int> >::iterator ti = rc.awin.begin();
  while ( ti != rc.awin.end() )
    {
      int count = 0;
      for ( int k = 0 ; k < ti->second.size(); k++)
	count += ti->second[k];
      
      O << setw(4) << PP->locus[ ti->first ]->chr << " "
	<< setw(par::pp_maxsnp ) << PP->locus[ ti->first ]->name << " "
	<< setw(12) << PP->locus[ ti->first ]->bp << " "
	<< setw(12) << PP->locus[ ti->first ]->freq << " "
	<< setw(12) << PP->locus[ ti->first ]->allele1 << " "
	<< setw(12) << count << "\n";		  
      ++ti;
    }
}
...
//     error("Could not find SNP field in header");
  ...
  map<int2,double>::iterator ti = pmap.begin();
  int srange_cnt = 0;
  bool inRange = false;
  Range srange;
  int l = 0;
  ...
  while ( ti != pmap.end() )
    {
      ...
      double pvalue = ti->second;
      
      double coef;
      if ( no_beta ) 
	coef = 99 ; 
      else 
	coef = bmap.find( ti->first )->second;
	   ...
	   && coef > 1 )
	{
	  inRange = true;
	  bestp = pvalue;
	  srange.chr = ti->first.p1;
	  srange.start = srange.stop = ti->first.p2;
	}
      else if ( inRange 
		&& ( pvalue > par::rare_test_score_range_threshold  || 
		     coef < 1 || 
		     ti->first.p1 != srange.chr ||
		     l == ntot ) )
	{
...
void Plink::elfBaseline()
{
  ...
  for (int i = 0 ; i < n ; i++ ) 
      ...
      for (int l = 0; l < nl_all; l++)
	{
      ...
      map<int,int>::iterator ti = chr_cnt.begin();
      while ( ti != chr_cnt.end() )
	{
	  int c = ti->first;
