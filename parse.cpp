...
//     { par::locked = false; }
  ...
  if ( a.find("--sglev")){
   par::sglev=true;
   par::SGLEV=a.value_double("--sglev");
  }
  if ( a.find("--upper"))
  ...
  if (a.find("--lamp-ld-remove"))
   {
      par::lamp_ld_remove=true;
      if( ! a.find("--comb"))
        error("Must specify --comb {comb_file_name} also if --lamp-ld-remove used");
      if( ! a.find("--file") && ! a.find("--bfile") \
                && ( ! (a.find("--ped") || a.find("--map"))) \
                && ! (a.find("--bim") || a.find("--bed") || a.find("--fam") ))
        error("Use either --file {root} OR --bfile {root} OR --ped {name} --map {name} OR --bed {name} --bim {name} --fam {name}");
  ...
  if( a.find("--comb"))
   {
      par::read_combfile = true;
      par::combfilename = a.value("--comb");
   }

  if( a.find("--lamp-r2"))
   par::LAMP_R2=a.value_double("--lamp-r2");
